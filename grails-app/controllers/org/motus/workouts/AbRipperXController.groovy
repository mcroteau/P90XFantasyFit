...
class AbRipperXController {
	...
	def logWorkout(){
		withBaseLogWorkout { account, workoutPlan, view ->
			def plannedWorkout = new AbRipperX(params)
			plannedWorkout.includeAbRipper = true
			request.plannedWorkout = plannedWorkout

			def previous = AbRipperX.findByWorkoutPlan(workoutPlan, [sort: "actualWorkoutDate", order: "desc"])
	...
	def entry(Long id){
		withBaseEntry { account, plannedWorkout, view ->
			request.plannedWorkout = plannedWorkout
			request.account = account
			def previous = AbRipperX.findByWorkoutPlan(plannedWorkout.workoutPlan, [sort: "actualWorkoutDate", order: "desc"])
	...
	def changeDate(Long id){
		withPlannedWorkout { abRipperXInstance, account ->
			request.plannedWorkout = abRipperXInstance
	...
	def skip(Long id){
		withBaseSkip { abRipperXInstance ->
	...
	def recover(Long id){
		withBaseRecover { abRipperXInstance ->
	...
	def remove(Long id){
		withBaseRemove { abRipperXInstance ->
	...
	def save(){
		withBaseSave { account, workoutPlan ->
			...
			def abRipperXInstance = new AbRipperX()
			
			abRipperXInstance.properties = params
			abRipperXInstance.includeAbRipper = true
			
			abRipperXInstance.actualWorkoutDate = params.actualWorkoutDate
			abRipperXInstance.plannedWorkoutDate = params.actualWorkoutDate
			abRipperXInstance.originalPlannedWorkoutDate = params.actualWorkoutDate
			
			abRipperXInstance.workoutPlan = workoutPlan
			abRipperXInstance.account = account
			abRipperXInstance.save(flush:true)
			
			workoutPlan.addToPlannedWorkouts(abRipperXInstance)
			workoutPlan.save(flush:true)
        	
			accoladeService.processWorkoutLogged(account, abRipperXInstance)
			accoladeService.processAbRipperLogged(account, abRipperXInstance)
			
			calculateAccountStats(account, workoutPlan)
			calculateWorkoutPlanStats(account, workoutPlan)
			checkAddToCompetitionStats(abRipperXInstance, account)
			updateCompetitionStatsAndStandings(account)
			sendMotivationMessages(account)
			
			def accolades = Accolade.findAllByPlannedWorkout(abRipperXInstance)
			def points = calculatePoints(accolades)
			
			checkCompletedWorkoutPlan(account, abRipperXInstance, abRipperXInstance.workoutPlan)
			
			flash.message = '<strong>+' + points + ' points. Good Job!</strong> Successfully completed ' + abRipperXInstance.displayName + '.  <a href="/bringit/' + abRipperXInstance.link + '/show/' + abRipperXInstance.id + '">Review Workout</a>'	
